...
class NotificationClient(discord.Client):
    async def on_ready(self):
        ...
        channel_share = self.get_channel(CHANNEL_ID_SHAREHOUSE)
        await channel_share.send("📢 鯖建てたよ～ ▶")
        channel_sio = self.get_channel(CHANNEL_ID_SIOKONBU)
        await channel_sio.send("📢 鯖建てたよ～ ▶")
